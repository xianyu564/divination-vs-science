# Sponsorship & Support

## 💝 Sponsorship Nature

**Important Note**: Sponsorship for this project is **NOT for profit**, but rather support and recognition for the author's research work. Payment expresses your support for the author's academic exploration and knowledge sharing.

## 🎯 Support Usage

Your support will be used for:
- 📚 Continuous updates of lecture content and academic materials
- 🛠️ Development of open source tools and templates  
- 🎪 Organizing offline/online sharing events
- 🔬 Supporting related academic research
- 📖 Maintaining open source community resources
- 🚀 **Supporting startup project development**
- 🌟 **Enhancing personal quality of life**

## 🌟 Sponsorship Methods

### GitHub Sponsors
[![GitHub Sponsors](https://img.shields.io/badge/GitHub%20Sponsors-Support%20Me-red?style=for-the-badge&logo=github)](https://github.com/sponsors/xianyu564)

### Alternative Payment Methods
- **WeChat Pay**: Scan QR code below
- **PayNow**: Singapore local bank transfer

<div align="center">
  <img src="../.github/WeChat Sponsor Code.jpg" alt="WeChat Sponsor Code" width="200" />
  <img src="../.github/SG PayNow Sponsor Code.jpg" alt="PayNow Sponsor Code" width="200" />
</div>

## 🏆 Donor Benefits

### Recognition in New Releases
- All donors will be specially thanked when new versions of the project are released
- Donor names will be listed in Release Notes
- Your contributions will be recorded in project update logs

### Inclusion in Project Donor List
- Your name will be permanently recorded in `docs/sponsors-list.md`
- You can choose to display your GitHub username or real name
- Categorized display by donation amount and time

### Special Benefits
- **Small Donors (≤$10)**: Name listed in donor list
- **Medium Donors ($11-$50)**: Special thanks in Release Notes
- **Large Donors ($51+)**: Special acknowledgment on project homepage and documentation
- **Continuous Donors**: Receive "Continuous Supporter" badge

## 📋 Transparency Commitment

- All sponsorship funds are used only for the above academic and community purposes
- Regular disclosure of fund usage in project status files
- Maintain the non-commercial nature and openness of the project
- Quarterly financial usage reports

## 📊 Fund Usage Report

### 2025 Q3 (Planned)
- Lecture content updates: 30%
- Open source tool development: 25%
- Startup project support: 25%
- Personal quality of life enhancement: 20%

### Fund Allocation Principles
- 70% for project development and community building
- 30% for supporting personal development and startup projects

## 🤝 Contact Information

If you have any questions or suggestions about sponsorship:
- Ask in GitHub Issues
- Send email to z_zz@u.nus.edu
- Leave messages through GitHub Sponsors

## 🙏 Acknowledgments

Thank you to all supporters for your trust and support! Every contribution makes this project better and allows me to continue exploring and sharing knowledge.

---

**Note**: Sponsorship is voluntary, and we are deeply grateful regardless of the amount. Your support will help this project continue to develop and bring value to more people.
